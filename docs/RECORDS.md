...
- **確定境界は Enter（改行）のみ**（2026-07-05 変更）。保存（デバウンス）時に
  `freezeLiveTail` が、Enter で完結し変換 settled な**行**をリテラルへ畳む。句点（。！？）
  では畳まない（複数文でも Enter までは 1 チャンク）。行区切りの改行はリテラルの外に
  残し、`chunkText` がチャンク境界として解釈する。
...
- 確定チャンク（メイン本文の各行）をクリック → 修正モードへ。元テキスト（確定後の
  漢字かな）をバッファに読み込み、**プレーンテキストとしてカーソル編集**する
  （`src/tui/App.tsx` の `editing` / `commitEdit`、編集ロジックは `applyEditKey`
  `src/tui/controller.ts`）。←→ / Home / End で移動、Backspace / Delete で削除。
- 編集モードは**変換しない**（既に確定済みテキストの手直しのため。打鍵がそのまま入る）。
  入力モード（追記専用＋ローマ字変換）とは別系統で、`Editor` コンパウンド
  （`src/tui/editor.tsx`）の `variant="edit"`（可動カーソル）/ `variant="new"`（追記）で
  描画を共有する。
- 確定で、その**凍結リテラル領域を編集後テキストで置換**（空確定なら削除）。
...
**改行（Enter）の決定的区切りのみ**（`chunkText` `src/chunk/chunker.ts` の一次区切り。
2026-07-05 に句点区切りを廃止し、ペースト領域は内部の改行でも分割しない）。
embedding は関連表示・セマンティック検索でのみ使う。
...
ここで予告した「変換を OS IME へ委譲しても**記録形式（確定テキスト）は同一**で、
入力部（ライブ末尾の生成）だけ差し替えればよい」は、**web で実際に起きた**
（2026-09-20, issue #149）。web の Composer は変換を持たず、ライブ末尾は
ローマ字打鍵ログではなく確定済みの素のテキストになったが、凍結リテラル・行グループ・
チャンク化はそのまま動いている（差し替えたのは Enter 確定時の `freezePlainTail` =
`apps/web/src/client/composer/plain-input.ts` だけ。打鍵の受付は OS の textarea 任せ）。

TUI は端末が IME プリエディットを扱えない（`CONCEPT.md` §形態）ため自前変換を維持する。
...
- `src/tui/App.tsx`: 確定チャンクを 1 行ずつクリック可能に描画＋ライブ末尾入力、
  保存時に `freezeLiveTail` で凍結、クリックで修正モード（カーソル編集・変換なし）。
...
- 修正モードはカーソル編集だが**変換を伴わない**プレーン編集。文中の一語を直すには
  目的の表記をそのまま打つ（確定済みテキストの手直しのため再変換しない）。
- 凍結時の変換は settled を待つ（未変換のうちは畳まない）。確定文の変換が
  遅延中はライブのまま残り、変換到着後に凍結される。**TUI のみ**: web は変換が無いので
  改行に達した行が常にその場で凍結される（issue #149）。
