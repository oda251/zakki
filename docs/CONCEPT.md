...
### 1. ローマ字入力 → 自動かな漢字変換（TUI）

ユーザーは IME を使わずローマ字を打つだけ。変換操作（スペースキー、候補選択、確定）を要求しない。

> 2026-09-20 追記（issue #149）: これは **TUI の形態**。端末は IME のプリエディットを扱えないため
> 自前変換を持つ。Web 版は OS の IME が使えるので変換を持たず、確定済みの文字列をそのまま受け取る
> （`COMPOSER.md` 軸2）。記録モデル（確定テキストの凍結）は両者で同一（`RECORDS.md`）。
...
- 一次区切り: 改行（Enter）のみ・決定的（2026-07-05 変更: 句点では区切らない。
  ペースト塊は内部に改行・句点があっても 1 チャンク）
- 二次区切り: 話題転換の検出
