...
退会後もセッション JWT は署名としては最長 12 時間有効なままなので、`requireSession` の直後に **`accounts` にその ID が実在するか**を確かめる（`requireLiveAccount`）。これが無いと退会直後の `GET /me/db` を `ensureUserDatabase` が「初回」と解釈し、消したはずの DB を作り直してしまう。恒久的な失効機構（端末単位のログアウト・鍵ローテーションでの一斉失効）は issue #117。
...
- **退会（`DELETE /me`）の即時性も同じキャッシュ窓の影響を受ける**。コントロールプレーンは退会直後から 401 を返す（`requireLiveAccount`）が、退会前に解決済みのセッションは中継サーバのキャッシュが切れるまで（最大 60 分）中継が通る。参照先の DB は既に消えているので読めるデータは無いが、即時の遮断は issue #117（恒久的な失効機構）の担当。
- 退会の UI 導線が無い（`DELETE /me` を直接叩く）。
