...
export interface EntrySnapshot {
  /** ローカル日付 YYYY-MM-DD（sessionId 省略時のデフォルトセッション解決に使う） */
  date: string;
  /** 保存先セッション。省略時は date のデフォルトセッション（TUI の従来挙動） */
  sessionId?: number;
  raw: string;
  converted: string;
  chunks: ChunkInput[];
...
/** snapshot の保存先セッションを解決する（省略時はデフォルトセッションを取得/作成） */
function resolveSession(
  db: Db,
  snapshot: Pick<EntrySnapshot, "date" | "sessionId">,
  now: string,
...
export function getOrCreateEntry(
  ...
  return getOrCreateDefaultSession(db, date, now).andThen((session) =>
    tryDbAsync(async () => {
...
export function saveSnapshot(
  ...
  return resolveSession(db, snapshot, now).andThen((session) =>
    tryDbAsync(() =>
...
/** entry のチャンクを position 順に読み、復号して SavedEntry に束ねる */
async function readEntryChunks(db: Db, entry: Entry): Promise<SavedEntry> {
  const crypto = getCrypto(db);
  const list = await db
    .select()
    .from(chunks)
...
/**
 * 日付のデフォルトセッションの entry を読む（TUI の従来挙動）。
...
export function getSessionEntryWithChunks(
  db: Db,
  sessionId: number,
  ...
  return tryDbAsync(async () => {
    ...
    return readEntryChunks(db, entry);
  });
}
