...
ファイルは Web UI からアップロードでき、1 チャンクとして日記に並ぶ（画像は復号した
画像として、それ以外はファイル名で表示。issue #157）。実体は Cloudflare R2 に置き、
保存期間は「無期限 / 1日 / 7日 / 30日」から選ぶ。10 MiB 以上では無期限を選べず、
有限期限でも R2 の本体だけを削除してメタデータは残す。アップロードごとに
クライアントサイド暗号化を選べる（パスワードはユーザーごとに一律。左メニューの設定で変更する。
変更しても既存ファイルは再暗号化されない ── 鍵は封筒で間接化している）。TUI では blob
チャンクはファイル名だけが読み取り専用で並ぶ。
