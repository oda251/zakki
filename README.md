...
ファイルは Web UI からアップロードでき、1 チャンクとして日記に並ぶ（画像は画像として、
それ以外はファイル名で表示。issue #157）。実体は Cloudflare R2 に置き、アップロードごとに
クライアントサイド暗号化を選べる（パスワードはユーザごとに一律。左メニューの設定で変更する。
変更しても既存ファイルは再暗号化されない ── 鍵は封筒で間接化してある）。TUI では blob
